...
|---|---|---|---|
| 1 | 성장금융 | 서비스 = 성장금융 | 황민영 |
| 2 | BD 아젠다 | 서비스·요청유형·업무영역 어디든 `BD 아젠다` | 김민우 · 오유나 |
| 3 | 고객문의 | 서비스 = 고객문의 | 장혜원 |
| 4 | 지출관리 | 서비스 = 지출관리 | 장혜원 |
| 5 | 가이드·CX | 요청유형·업무영역에 `가이드`·`CX`·`고객 안내`·`응대` | 장혜원 |
| 6 | 카드-비씨·신한 | 서비스 = 카드 & 제휴사 = 비씨/BC/신한 | 김소은 |
| 7 | 카드-국민·롯데 | 서비스 = 카드 & 제휴사 = 국민/KB/롯데 | 김민지 |
| 8 | 카드-제휴사불명 | 서비스 = 카드 (그 외) | 김소은 · 김민지 |

**성장금융은 가이드 건이어도 황민영**이다(규칙 1이 5보다 위). 카드의 가이드 건만 장혜원으로 간다.
같은 이유로 **성장금융 + BD 아젠다도 황민영**으로 간다. 바꾸려면 `ROUTES`에서 두 줄의 순서만 바꾸면 된다.

BD 아젠다는 `serviceRaw`(폼 원문)도 함께 본다. `SERVICE` 매핑에 없는 새 선택지가
서비스 칸에 들어와도 잡히게 하기 위해서다.
...
| Linear 상태 | 스레드 알림 |
|---|---|
| In Progress / In Review | `▶️ {담당자} 님이 처리를 시작했습니다.` |
| Done | `✅ 처리 완료되었습니다.` |
| Canceled | `🚫 이 요청은 종료되었습니다.` |

Triage·Backlog·Todo는 알리지 않는다(접수 직후 기본 상태라 소음이 된다).

중복 발송은 **스레드에 그 문구가 이미 있는지**로 막는다. 상태 파일이 필요 없다.

### 이모지는 쓰지 않는다 (기본 OFF)

`reactions:write` 스코프가 없어 **스레드 코멘트가 기본이자 유일한 경로**다.
스코프를 붙이고 이모지를 쓰려면 `OPS_USE_REACTIONS=true` 환경변수를 켜면 된다.
켜지 않으면 실패할 API를 아예 호출하지 않는다.
