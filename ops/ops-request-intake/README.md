...
### 접수 시점에 Linear 담당자를 자동 배정하지 않는다

의도적이다. **Linear 담당자 배정이 곧 `시작(▶️)` 신호**이기 때문에,
접수와 동시에 배정해버리면 "아직 아무도 안 잡은 건"과 "누가 집어간 건"을 구분할 수 없게 된다.

흐름은 이렇다.
1. 접수 → 스레드에서 담당 후보를 멘션 (이모지 없음)
2. 담당자가 Linear에서 **자신을 배정** → ▶️
3. Done → ✅ / Canceled → 🚫

`OWNERS[].linear`는 향후 *미배정 에스컬레이션*(SLA 내 아무도 안 집으면 호출)에 쓰려고 남겨둔 값이다.
